...
//        arrayBlockingQueue.put("123");
        ...
        // 2.如果共享资源空闲，则通过 CLH锁（一种虚拟的双向队列） 来实现线程阻塞等待以及唤醒时锁分配
        ...
        // 2.set方法设置属性值
        // 3.如果实现*Aware接口，则执行调用对应的方法
        // 4.如果实现了InitializingBean接口，则执行afterPropertiesSet()方法
        // 5.执行init-method方法
        // 6.执行destroy()方法销毁
...
//         */
        ...
        // es的force merge 和 shrink操作
            // force merge：索引级别的操作，将多个分段合并为较少的分段来减少索引的存储空间，减少磁盘占用
                // es索引由多个分片组成，每个分片由多个段组成（段时lucene索引中的基本存储单元）
